...
public class World {
   ...
   public static String descriptionOfBit(int n) {
      return n == -1?"(Unused bit for spacing)":(n >= 0 && n < NWORLDBITS?((BitName)bitnameList.get(n)).description:"(Invalid world bit)");
   }

   public static String nameOfBit(int n) {
      return n == -1?"null":(n >= 0 && n < NWORLDBITS?((BitName)bitnameList.get(n)).name:"");
   }

   public static int bitNumberOf(String name) {
      int n;
      for(n = 0; n < NWORLDBITS && name.compareTo(((BitName)bitnameList.get(n)).name) != 0; ++n) {
         ;
      }
